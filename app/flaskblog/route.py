...
from flaskblog.forms import RegistrationForm, LoginForm, AddPlacesForm
from flaskblog.models import User, AddPlaces
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and form.password.data == user.password:
            ...
            return render_template('addplace.html', form=AddPlacesForm(), logo = full_filename)
...
def addplace():
    ...
    if form.validate_on_submit():
        ...
        addP = AddPlaces(city=form.cityname.data,name=form.placename.data,image = pic_file,desc=form.description.data,features=form.features.data)
        db.session.add(addP)
        db.session.commit()
        flash('Places has been added')
        return redirect(url_for('about'))
    return render_template('addplace.html', title='New Place', form=form, logo = full_filename)
